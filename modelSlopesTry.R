...
if (useMidPointCriteria) {
  ...
  checkIfGotCriterionObjectsCombo <- function(df,threshes) { #Printing whether anomalous case
    #hh<<-df
...
thrThisCrit$negativSlope = -thrThisCrit$slopeThisCrit #want up on graph to be steeper slope
#To plot threshes, set dv to "thresh". To plot slopes, use "negativSlope"
dv = "negativSlope" 
showIndividData=TRUE
tit<-dv
if (!showIndividData) {
  quartz(tit,width=4.5,height=4)
  h=ggplot(thrThisCrit,aes_string(x="limit",y=dv,color="type"))  
  h=h+theme_bw()+ facet_grid(targets~numObjects)
  dodge=position_dodge(width=0.2)
  h=h+stat_summary(fun.y=mean,geom="point",size=2.5)
  h=h+stat_summary(fun.data="mean_cl_boot",geom="errorbar",conf.int=.67,width=.2)
#  h=h+stat_summary(fun.data="mean_cl_boot",aes(group=targets),geom="errorbar",conf.int=.67,width=.2)
#  h=h+stat_summary(fun.y=mean,geom="point",aes(group=targets),size=2.5)
  h=h+stat_summary(fun.y=mean,geom="line")
} else {
  quartz(tit,width=8,height=7)  #(tit,width=4,height=3.5)
  h=ggplot(thrThisCrit,aes_string(x="limit",y=dv,color="type",shape="subject"))
  h=h+theme_bw()+ facet_grid(targets~numObjects)
  dodgeAmt=0.2
  h=h+geom_point(size=2.5,position=position_dodge(width=dodgeAmt))
  h=h+geom_line(aes(group=subject),position=position_dodge(dodgeAmt))
...
#NOT WORKING!
actual=threshes_speed_123targets269objects
if (useMidPointCriteria) {
  midPointCriteria = data.frame(numObjects= unique(threshes$numObjects))
  midPointCriteria$critToPlot = (1.0 + 1/midPointCriteria$numObjects) / 2.0
  actualThisCrit = merge(actual,midPointCriteria)
  #dataframe that indicate the appropriate crit to use. 
  #Then, take only rows where criterion and critToPlot match  
  actualEachSubject = actualThisCrit[actualThisCrit$critToPlot == actualThisCrit$criterion,  ]
} else {
  actualEachSubject = subset(actual,criterion==thisCrit)  
}


actualEachSubject$negativSlope= -actualEachSubject$slopeThisCrit #want up on graph to be steeper slope
